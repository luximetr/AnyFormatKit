...
//    return (formattedText: formattedText, caretBeginOffset: corretOffset)
    ...
    if text.isEmpty {
      let newRange = correctRangeForDeleting(from: currentText, at: internalRange) ?? NSRange(location: 0, length: 0)
...
//    }
    var newUnformatted = unformat(from: newString)
    
    newUnformatted = stringOnlyWithAllowedSymbols(from: newUnformatted)
    let newFormatted = format(text: newUnformatted)
    
    let caretOffset = rangeOffset(range: range, oldString: currentText, newString: newFormatted)
    return (formattedText: newFormatted, caretBeginOffset: caretOffset)
  }
...
//  }
  ...
  private func getCorrectedCaretPosition(range: NSRange, oldString: String, newString: String) -> Int {
    return rangeOffset(range: range, oldString: oldString, newString: newString)
  }
...
//  }
  ...
  private func correctRangeForDeleting(from string: String?, at range: NSRange) -> NSRange? {
    let unformatted = unformat(from: string ?? "")
  ...
  private func isCorrectInserting(from string: String?, at range: NSRange) -> Bool {
    ...
    let unformated = unformat(from: text)
